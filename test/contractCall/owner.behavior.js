...
function shouldBehaveLikeOwner (alice) {
  ...
  it('updateMasterMinter', async function () {
    expect(await this.token.masterMinter()).to.be.equal(this.contract.address);
    
    await this.contract.functionCall(
        this.token.address,
        this.token.contract.methods.updateMasterMinter(alice).encodeABI()
    );

    expect(await this.token.masterMinter()).to.be.equal(alice);
  });
  ...
  it('updateRescuer', async function () {
    expect(await this.token.rescuer()).to.be.equal(ZERO_ADDRESS);
