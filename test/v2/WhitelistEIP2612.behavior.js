...
function whitelistWithEIP2612(
  ...
  describe('permit', function () {
    ...
    it('accepts owner signature when unwhitelisted and under 100000 tokens', async function () {
      ...
      await this.token.unWhitelist(owner, { from: whitelister })
    ...
    it('reverts when permit with unwhitelisted account and over 100000 tokens', async function () {
    ...
    it('accepts owner signature when he is whitelisted and under 100000 tokens', async function () {
      const value = sendAmountBelow
      ...
      await this.token.whitelist(owner, { from: whitelister })
      const data = buildData(this.chainId, this.token.address)
      const signature = ethSigUtil.signTypedMessage(wallet.getPrivateKey(), {
        data,
      })
    ...
    it('accepts owner signature when he is whitelisted and over 100000 tokens', async function () {
      const value = sendAmountAbove
